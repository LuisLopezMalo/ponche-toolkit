...
namespace PT.Graphics3D.Effects
{
    ...
    public class PTMaterial : UpdatableStateObject, IInitializable, IContentLoadable
    {
        ...
        private List<TexturePath> texturePaths;
        private TexturePath[] defaultTexturePaths;
        ...
        /// <summary>
        /// The key to retrieve the common material for wood rendering from the <see cref="PTForwardRenderEffect.Materials"/> dictionary.
        /// </summary>
        public static readonly string COMMON_WOOD_MATERIAL_KEY = "common_wood_material";

        /// <summary>
        /// The key to retrieve the common material for metal rendering from the <see cref="PTForwardRenderEffect.Materials"/> dictionary.
        /// </summary>
        public static readonly string COMMON_METAL_MATERIAL_KEY = "common_metal_material";
        #endregion
        ...
        /// </summary>
        //public Dictionary<int, PTTexture2D> Textures
        ...
        /// <summary>
        /// This property will be filled when the <see cref="IsReflectivityEnabled"/> property is active.
        /// This is used to render to a texture the reflection scene.
        /// </summary>
        public RenderTargetView ReflectionRenderTargetView
        {
            get { return reflectionRenderTargetView; }
            internal set { SetProperty(ref reflectionRenderTargetView, value); }
        }
        ...
        public PTMaterial(Game11 game, params TexturePath[] defaultTexturePaths)
        {
            this.game = game;
            ...
            texturePaths = new List<TexturePath>();
        }
        #endregion
        ...
        public override bool UpdateState()
        {
            ...
            if (obj != null)
            {
                ...
                for (int i = 0; i < texturePaths.Count; i++)
                {
                    ...
                    {
                        textures[i].Dispose();
                        textures[i] = game.ContentManager.LoadTexture2D(texturePaths[i].Path);
                        textures[i].LoadContent(Game.ContentManager);
                    }
                    else
                    {
                        PTTexture2D texture = game.ContentManager.LoadTexture2D(texturePaths[i].Path);
        ...
        /// If the texture paths are added before calling the <see cref="LoadContent(ContentManager)"/> method,
        /// those textures are loaded there.
        /// If they are set as dirty and added after calling <see cref="LoadContent(ContentManager)"/>, they will be
        ...
        public void AddTexturePath(TexturePath texturePath, bool asDirty)
        {
            ...
            {
                foreach (TexturePath path in texturePaths)
                {
            ...
            if (asDirty)
                SetPropertyAsDirty(ref texturePaths, new List<TexturePath>(texturePaths), nameof(TexturePaths));
        }
        ...
        /// <param name="asDirty">The value to indicate if the property must be set dirty, so when the <see cref="UpdateState"/>
        /// method is called, the textures are created. If it set to false, the textures must be manually loaded into memory.</param>
        public void AddTexturePath(TexturePath texturePath, PTTexture2D.TextureType type, bool asDirty)
        {
        ...
        /// <param name="asDirty">The value to indicate if the property must be set dirty, so when the <see cref="UpdateState"/>
        /// method is called, the textures are created. If it set to false, the textures must be manually loaded into memory.</param>
        public void AddTexturePath(TexturePath texturePath, bool asDirty, int index = -1)
        {
        ...
        public void SetTexturePath(TexturePath texturePath, int index)
        {
            if (texturePaths.Count <= index)
            {
                Log.Error("The index of the texture path is out of bounds. -{0}-", this.Name);
        ...
        public void Apply(PTForwardRenderEffect effect, PTMesh mesh, DeviceContext1 context)
        {
            ...
            if (HasTexture)
            {
                ...
                context.PixelShader.SetSamplers(0, texture.Sampler);
            ...
            if (isReflectivityEnabled)
            {
                ...
                Matrix reflectiveMatrix = Game.CurrentCamera.GetReflectionMatrix(1);
                ...
                context.PixelShader.SetShaderResource(6, reflectionTexture.ShaderResourceView);
            }
        ...
        public override void Dispose()
        {
            ...
            {
                ContentsPool.ClearTexture(tex.Paths[0].Path.ToLower());
            }
