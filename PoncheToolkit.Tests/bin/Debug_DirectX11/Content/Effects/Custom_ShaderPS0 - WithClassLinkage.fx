...
#include "Custom_ShaderPS0_Classes.fx"
#endif

cbuffer MaterialConstantBuffer : register(b0)
{
    MaterialClass0 Material;
};

cbuffer GlobalDataConstantBuffer : register(b1)
{
    GlobalDataClass0 GlobalData;
};

cbuffer LightsConstantBuffer : register(b2) // 64 kb per ConstantBuffer MAX
{
    LightClass0 Lights[MAX_LIGHTS];
};

// ================================ IMaterial methods ================================
_BumpProperties MaterialClass0::CalculateBump(Texture2D<float4> bumpMapTexture, SamplerState textureSampler, float3 tangent, float3 binormal, float3 normal, float2 texCoord)
{
...
float4 MaterialClass0::CalculateSpecular(Texture2D specularMapTexture, SamplerState samplerState, float4 currentSpecular, float2 texCoord)
{
...
float4 MaterialClass0::CalculateDiffuse(float4 globalAmbient, float4 lightDiffuse, float4 lightSpecular)
{
...
// ================================ ILight methods ================================
_LightCalculationResult LightClass0::CalculateLight(IMaterial mat, float4 position, float3 viewDirection,
...
float4 LightClass0::CalculateDiffuse(IMaterial mat, float3 lightDirection, float3 normal, float3 bumpNormal, out float diffuseIntensity)
{
    float diffuse;
    ...
    float4 resultColor = float4(0, 0, 0, 0);
    if (diffuse > 0.988)
        resultColor = float4(1, 1, 1, 1) * Col;
    else if (diffuse > 0.6)
        resultColor = float4(0.7, 0.7, 0.7, 1.0) * Col;
    else if (diffuse > 0.1)
        resultColor = float4(0.4, 0.4, 0.4, 1.0) * Col;
    else if (diffuse > 0)
        resultColor = float4(0.1, 0.1, 0.1, 1.0) * Col;

    return resultColor * Intensity * diffuse;
}

// Calulcate the specular lighting.
float4 LightClass0::CalculateSpecular(IMaterial mat, float3 viewDirection, float3 lightDirection, float3 normal, float3 bumpNormal, float2 texCoord,
                            Texture2D<float4> specularMap, SamplerState textureSampler)
{
    ...
    if (mat.GetIsBump())
        NormalDotHalf = max(0, dot(bumpNormal, halfVector));
    else
        NormalDotHalf = max(0, dot(normal, halfVector));

    //float4 result = Color * pow(saturate(NormalDotHalf), mat.GetSpecularPower());
    float4 result = Col * pow(NormalDotHalf, mat.GetSpecularPower());
    if (mat.GetHasSpecularMap())
        result *= specularMap.Sample(textureSampler, texCoord);

    //if (dotProduct > 0.9)
    //    result = float4(0, 1, 0, 1) * result;
    //else if (dotProduct > 0.6)
    //    result = float4(1, 0, 0, 1.0) * result;
    //else if (dotProduct > 0.1)
    //    result = float4(0.4, 0.4, 0.4, 1.0) * result;
    //else if (dotProduct > 0)
    //    result = float4(0.1, 0.1, 0.1, 1.0) * result;

    return result * Intensity;
}

float LightClass0::CalculateSpotCone(float3 lightVector)
{
...
_LightCalculationResult LightClass0::CalculateDirectionalLight(IMaterial mat, float4 position, float3 viewDirection,
                            float3 normal, float3 bumpNormal, float2 texCoord, Texture2D<float4> specularMap, SamplerState textureSampler)
{
    _LightCalculationResult result = { { 0, 0, 0, 0 }, { 0, 0, 0, 0 }, 0, false };
...
_LightCalculationResult LightClass0::CalculatePointLight(IMaterial mat, float4 position, float3 viewDirection,
                            float3 normal, float3 bumpNormal, float2 texCoord, Texture2D<float4> specularMap, SamplerState textureSampler)
{
    _LightCalculationResult result = { { 0, 0, 0, 0 }, { 0, 0, 0, 0 }, 0, false };
    ...
    lightVector = lightVector / distance; // = normalize(lightVector);
...
_LightCalculationResult LightClass0::CalculateSpotLight(IMaterial mat, float4 position, float3 viewDirection,
                            float3 normal, float3 bumpNormal, float2 texCoord, Texture2D<float4> specularMap, SamplerState textureSampler)
{
    _LightCalculationResult result = { { 0, 0, 0, 0 }, { 0, 0, 0, 0 }, 0, false };

    //float3 lightVector = normalize(GetPosition() - position).xyz;
    float3 lightVector = (Pos - position).xyz;
    float distance = length(lightVector);
...
float LightClass0::CalculateAtt(float distance)
{
...
// ========================== GLOBAL DATA ===============
float4 GlobalDataClass0::ProcessFinalColor(float4 finalColor)
{
    float gray = ((0.2126 * finalColor.r) + (0.7152 * finalColor.g) + (0.0722 * finalColor.b));
    return float4(gray, gray, gray, 1);
}

int GlobalDataClass0::GetCurrentLights()
{
    return CurrentLights;
}

float4 GlobalDataClass0::GetGlobalAmbient()
{
