...
namespace PoncheToolkit.Util
{
    ...
    public class WICHelper
    {
        ...
        public static Texture2D CreateTexture2DFromBitmap(SharpDX.Direct3D11.Device device, SharpDX.WIC.BitmapSource bitmapSource, bool generateMipMaps)
        {
            ...
            using (var buffer = new SharpDX.DataStream(bitmapSource.Size.Height * stride, true, true))
            {
                ...
                if (generateMipMaps)
                {
                    Texture2DDescription desc = new Texture2DDescription()
                    {
                        ...
                        MipLevels = 0,
                        OptionFlags = ResourceOptionFlags.GenerateMipMaps, // OptionFlags = ResourceOptionFlags.SharedNthandle | ResourceOptionFlags.SharedKeyedmutex,
                        SampleDescription = new SharpDX.DXGI.SampleDescription(1, 0),
                    };

                    // Update the first mip-level (0) so the mipmaps are generated when the Context.GenerateMipMaps is called.
                    SharpDX.Direct3D11.Texture2D texture = new SharpDX.Direct3D11.Texture2D(device, desc);
                    DataBox box = new DataBox(buffer.DataPointer, stride, 0);
                    device.ImmediateContext.UpdateSubresource(box, texture, 0);
                ...
                else
                {
                    ...
                    return new SharpDX.Direct3D11.Texture2D(device, desc, new SharpDX.DataRectangle(buffer.DataPointer, stride));
                }
            }
        }
        ...
        /// <returns>A Texture2D</returns>
        public static Texture2D CreateTexture2DFromBitmap(SharpDX.Direct3D11.Device device, List<SharpDX.WIC.BitmapSource> bitmapSources, bool generateMipMaps)
        {
            //==== TODO: implement generation of mip maps
        ...
        public static Graphics2D.PTTexture2D LoadTextureFromFile(SharpDX.Direct3D11.Device device, string fileName, bool generateMipMaps)
        {
            BitmapSource bitmap = WICHelper.LoadBitmapSource(fileName);
            Texture2D texture = WICHelper.CreateTexture2DFromBitmap(device, bitmap, generateMipMaps);
            bitmap.Dispose();
        ...
        /// <param name="generateMipMaps">If the texture will generate mip maps.</param>
        /// <returns>Return a custom Texture2D PoncheToolkit object with the texture inside.</returns>
        public static Graphics2D.PTTexture2D LoadTextureFromFile(SharpDX.Direct3D11.Device device, SharpDX.Direct2D1.DeviceContext context, bool generateMipMaps, string[] texturePaths)
        {
            ...
            {
                result.SetTexturePath(new PTTexturePath(path), false);
            ...
            Texture2D texture = WICHelper.CreateTexture2DFromBitmap(device, bitmaps, generateMipMaps);
        ...
        /// </summary>
        /// <param name="device"></param>
        /// <param name="context2D">The <see cref="SharpDX.Direct2D1.DeviceContext"/> to be used to load the Bitmap.</param>
        /// <param name="fileName">File name of the texture to be loaded.</param>
        /// <param name="generateMipMaps">If the texture will generate mip maps.</param>
        /// <returns>Return a custom Texture2D PoncheToolkit object with the texture inside.</returns>
        public static Graphics2D.PTTexture2D LoadTextureFromFile(SharpDX.Direct3D11.Device device, SharpDX.Direct2D1.DeviceContext context2D, string fileName, bool generateMipMaps)
        {
            SharpDX.Direct2D1.Bitmap bitmapResult = null;
            using (BitmapDecoder bitmapDecoder = new SharpDX.WIC.BitmapDecoder(factory, fileName, DecodeOptions.CacheOnDemand))
            {
                ...
                BitmapSource bitmap = WICHelper.LoadBitmapSource(bitmapDecoder, context2D, out bitmapResult);
                Texture2D texture = WICHelper.CreateTexture2DFromBitmap(device, bitmap, generateMipMaps);
                Graphics2D.PTTexture2D result = new Graphics2D.PTTexture2D(device, texture);
                result.SetTexturePath(new Graphics3D.PTTexturePath(fileName), false);
                result.Bitmap = bitmapResult;
                result.BitmapSourceEffect = new SharpDX.Direct2D1.Effects.BitmapSource(context2D);
                result.BitmapSourceEffect.WicBitmapSource = bitmap;
                result.BitmapSourceEffect.Cached = true;
