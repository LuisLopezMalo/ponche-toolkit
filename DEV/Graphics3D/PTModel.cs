...
namespace PoncheToolkit.Graphics3D
{
    ...
    public class PTModel : GameRenderableComponent, ICloneable, IContentItem, IInputReceivable
    {
        ...
        /// If the model is a primitive or something else, it is calculated once when the <see cref="LoadContent(IContentManager)"/>
        /// method is called.
        /// </summary>
        [Util.Reflection.PTSerializableProperty]
        ...
        public void SetMaterial(PTMaterial material, PTMesh mesh)
        {
            if (string.IsNullOrEmpty(material.Name))
                throw new ArgumentNullException("The material to be applied has no name.");
            mesh.SetMaterial(material.Name);
        }
        ...
        public void SetEffect(PTForwardRenderEffect effect, int index)
        {
            this.Effects[index] = effect;
            foreach (PTMesh mesh in meshes)
            {
                mesh.Effects = this.Effects;
            }
        }
        ...
        public static PTModel FromAssimpScene(ref Scene scene, ref PTModel model, string modelPath)
        {
            ...
            // Convert the assimp scene to local objects.
